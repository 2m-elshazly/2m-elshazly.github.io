...
// =====================================================
// 2M Elshazly - إشعارات الموقع
// ملاحظة: لوحة الإدارة تحفظ الإشعار في notifications.
// هذا الجزء يستمع لها لحظيًا للعميل المسجل دخوله.
// =====================================================
let stopNotifications = null;

function installNotificationUI() {
  if (document.getElementById("m2mNotificationBell")) return;

  const style = document.createElement("style");
  style.id = "m2mNotificationStyle";
  style.textContent = `
    #m2mNotificationWrap{position:fixed;top:16px;right:18px;z-index:99999;font-family:Arial,sans-serif;direction:rtl}
    #m2mNotificationBell{border:1px solid rgba(255,255,255,.18);background:#111;color:#fff;width:46px;height:46px;border-radius:50%;cursor:pointer;font-size:21px;box-shadow:0 8px 25px rgba(0,0,0,.35);position:relative}
    #m2mNotificationBadge{position:absolute;top:-4px;left:-4px;background:#e11d48;color:#fff;min-width:20px;height:20px;padding:0 5px;border-radius:20px;font-size:11px;font-weight:900;display:none;align-items:center;justify-content:center;border:2px solid #111}
    #m2mNotificationPanel{display:none;position:absolute;top:54px;right:0;width:min(360px,calc(100vw - 30px));max-height:70vh;overflow:auto;background:#151515;color:#fff;border:1px solid #2c2c2c;border-radius:18px;box-shadow:0 18px 60px rgba(0,0,0,.55);padding:12px}
    #m2mNotificationPanel.open{display:block}
    .m2mNotifHead{display:flex;align-items:center;justify-content:space-between;padding:4px 4px 10px;border-bottom:1px solid #282828;margin-bottom:8px}
    .m2mNotifHead b{font-size:15px}
    .m2mNotifItem{padding:12px;border:1px solid #282828;border-radius:13px;background:#101010;margin-bottom:8px;cursor:pointer}
    .m2mNotifItem.unread{border-color:#0070d1;background:rgba(0,112,209,.10)}
    .m2mNotifItem b{display:block;font-size:13px;margin-bottom:5px}
    .m2mNotifItem p{margin:0;color:#aaa;font-size:12px;line-height:1.7}
    .m2mNotifItem small{display:block;color:#666;margin-top:7px;font-size:10px}
    .m2mNotifEmpty{padding:20px;text-align:center;color:#777;font-size:12px}
  `;
  ...
  wrap.innerHTML = `
    ...
    <div id="m2mNotificationPanel">
      <div class="m2mNotifHead"><b>🔔 إشعارات 2M Elshazly</b></div>
      <div id="m2mNotificationList"><div class="m2mNotifEmpty">لا توجد إشعارات جديدة.</div></div>
    </div>
  `;
  document.body.appendChild(wrap);

  document.getElementById("m2mNotificationBell").addEventListener("click", e => {
    e.stopPropagation();
    document.getElementById("m2mNotificationPanel").classList.toggle("open");
  });
  document.addEventListener("click", e => {
    if (!wrap.contains(e.target)) document.getElementById("m2mNotificationPanel")?.classList.remove("open");
  });
}
...
function startNotifications(user, customerData) {
  ...
  const listEl = document.getElementById("m2mNotificationList");
  const badgeEl = document.getElementById("m2mNotificationBadge");
  const customerTypeValue = customerData?.customerType || "";

  // نقرأ الإشعارات الشخصية فقط + إشعارات broadcast.
  // لا نستخدم OR query هنا لتجنب الحاجة لفهرس مركب.
  const q = query(collection(db, "notifications"), where("target", "==", "all"));

  const stopAll = onSnapshot(q, async snapshot => {
    const all = snapshot.docs.map(d => ({id:d.id, ...d.data()}));
    const personal = await getDocs(query(collection(db, "notifications"), where("targetUserId", "==", user.uid)));
    const personalArr = personal.docs.map(d => ({id:d.id, ...d.data()}));

    const merged = [...all, ...personalArr]
      .filter((n,i,a) => a.findIndex(x=>x.id===n.id)===i)
      .filter(n => {
        if (n.targetUserId === user.uid) return true;
        if (n.target !== "all") return false;
        if (!n.customerType) return true;
    ...
    const unread = merged.filter(n => n.read !== true);
    badgeEl.textContent = unread.length > 99 ? "99+" : String(unread.length);
    badgeEl.style.display = unread.length ? "flex" : "none";

    listEl.innerHTML = merged.length ? merged.map(n => `
      <div class="m2mNotifItem ${n.read ? "" : "unread"}" data-notif-id="${n.id}">
        <b>${escapeNotificationText(n.title || "إشعار جديد")}</b>
        <p>${escapeNotificationText(n.message || "")}</p>
        <small>${notificationDate(n.createdAt)}</small>
      </div>
    `).join("") : '<div class="m2mNotifEmpty">لا توجد إشعارات حتى الآن.</div>';
    ...
    listEl.querySelectorAll("[data-notif-id]").forEach(item => {
      item.addEventListener("click", async () => {
        ...
        try {
          if(n && n.read !== true) await updateDoc(doc(db,"notifications",id), {read:true});
          if(n?.link) window.location.href=n.link;
        } catch(error) {
          console.error("2M notification read error:", error);
        }
      });
    });
  }, error => {
    console.error("2M notifications listener error:", error);
  });

  stopNotifications = () => stopAll();
}
...
onAuthStateChanged(auth, async (user) => {
  ...
  try {
    const userData = await prepareUserDocument(user);
    startNotifications(user, userData);
